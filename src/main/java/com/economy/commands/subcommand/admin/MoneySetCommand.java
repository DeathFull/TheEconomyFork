...
public class MoneySetCommand extends AbstractAsyncCommand {

    private RequiredArg<String> targetName;
    private RequiredArg<Double> amount;

    public MoneySetCommand() {
        super("set", com.economy.util.LanguageManager.getTranslation("desc_money_set"));
        this.setPermissionGroup(GameMode.Creative);
        this.targetName = this.withRequiredArg("nick", "Nome do jogador", ArgTypes.STRING);
        this.amount = this.withRequiredArg("valor", "Valor a definir", ArgTypes.DOUBLE);
        
        // Define a permissão do comando via reflexão para aparecer no LuckPerms
        com.economy.util.CommandPermissionHelper.setCommandPermission(this, com.economy.commands.CommandMessages.PERMISSION_ADMIN_MONEY_SET);
    }
    ...
    protected CompletableFuture<Void> executeAsync(CommandContext commandContext) {
        ...
        if (!com.economy.util.PermissionHelper.hasPermission(sender, com.economy.commands.CommandMessages.PERMISSION_ADMIN_MONEY_SET)) {
        ...
        if (targetNameValue == null || targetNameValue.isEmpty()) {
            sender.sendMessage(com.economy.commands.CommandMessages.USAGE_SET());
            return CompletableFuture.completedFuture(null);
        }
        ...
        if (sender instanceof Player player) {
            ...
            if (ref != null && ref.isValid()) {
                ...
                return CompletableFuture.runAsync(() -> {
                    executeSetCommand(sender, targetNameValue, amountValue);
                }, world);
            } else {
    ...
    private void executeSetCommand(CommandSender sender, String targetNameValue, Double amountValue) {
        ...
        EconomyManager.getInstance().setBalance(targetUuid, amountValue);
        String playerName = EconomyManager.getInstance().getPlayerName(targetUuid);
        sender.sendMessage(com.economy.commands.CommandMessages.BALANCE_SET());
        java.util.Map<String, String> placeholders = new java.util.HashMap<>();
        placeholders.put("player", playerName);
        placeholders.put("amount", com.economy.util.CurrencyFormatter.formatNumberOnly(amountValue));
        sender.sendMessage(com.economy.util.LanguageManager.getMessage("chat_balance_set_to", java.awt.Color.GREEN, placeholders));
        ...
        if (targetPlayerRef != null) {
            ...
            com.hypixel.hytale.server.core.Message message = com.economy.util.LanguageManager.getMessage("chat_balance_set_notification", java.awt.Color.GREEN, receivedPlaceholders);
