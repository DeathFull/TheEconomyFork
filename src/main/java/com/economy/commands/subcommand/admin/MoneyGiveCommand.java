...
public class MoneyGiveCommand extends AbstractAsyncCommand {

    private RequiredArg<String> targetName;
    private RequiredArg<Double> amount;

    public MoneyGiveCommand() {
        super("give", com.economy.util.LanguageManager.getTranslation("desc_money_give"));
        this.setPermissionGroup(GameMode.Creative);
        this.targetName = this.withRequiredArg("nick", "Nome do jogador", ArgTypes.STRING);
        this.amount = this.withRequiredArg("valor", "Valor a adicionar", ArgTypes.DOUBLE);
        
        // Define a permissão do comando via reflexão para aparecer no LuckPerms
        com.economy.util.CommandPermissionHelper.setCommandPermission(this, com.economy.commands.CommandMessages.PERMISSION_ADMIN_MONEY_GIVE);
    }
    ...
    protected CompletableFuture<Void> executeAsync(CommandContext commandContext) {
        ...
        if (!com.economy.util.PermissionHelper.hasPermission(sender, com.economy.commands.CommandMessages.PERMISSION_ADMIN_MONEY_GIVE)) {
        ...
        if (targetNameValue == null || targetNameValue.isEmpty()) {
            sender.sendMessage(com.economy.commands.CommandMessages.USAGE_GIVE());
            return CompletableFuture.completedFuture(null);
        }
    ...
    private void executeGiveCommand(CommandSender sender, String targetNameValue, Double amountValue) {
        ...
        double newBalance = EconomyManager.getInstance().getBalance(targetUuid);
        sender.sendMessage(com.economy.commands.CommandMessages.BALANCE_ADDED());
        java.util.Map<String, String> placeholders = new java.util.HashMap<>();
        placeholders.put("amount", com.economy.util.CurrencyFormatter.formatNumberOnly(amountValue));
        placeholders.put("player", playerName);
        placeholders.put("balance", com.economy.util.CurrencyFormatter.formatNumberOnly(newBalance));
        sender.sendMessage(com.economy.util.LanguageManager.getMessage("chat_balance_added_to", java.awt.Color.GREEN, placeholders));
        ...
        if (targetPlayerRef != null) {
            ...
            com.hypixel.hytale.server.core.Message message = com.economy.util.LanguageManager.getMessage("chat_money_received", java.awt.Color.GREEN, receivedPlaceholders);
