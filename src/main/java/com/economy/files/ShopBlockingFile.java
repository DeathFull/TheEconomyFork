...
public class ShopBlockingFile extends BlockingDiskFile {

    private ShopTracker tracker;
    private static final Gson GSON = new GsonBuilder().setPrettyPrinting().create();

    public ShopBlockingFile() {
        super(Path.of(FileUtils.SHOP_PATH));
    ...
    protected void read(BufferedReader bufferedReader) throws IOException {
        ...
        // Lê tabs se existir (compatibilidade com versões antigas)
        if (root.has("Tabs")) {
            JsonArray tabsArray = root.getAsJsonArray("Tabs");
            if (tabsArray != null) {
                tabsArray.forEach(jsonElement -> {
        ...
        itemsArray.forEach(jsonElement -> {
            ...
            // Lê tab se existir (compatibilidade com versões antigas)
            if (itemObj.has("Tab")) {
                item.setTab(itemObj.get("Tab").getAsString());
            } else {
                item.setTab("");
            }
            // Lê campos de comando console se existirem (compatibilidade com versões antigas)
