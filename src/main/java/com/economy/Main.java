...
import com.economy.commands.CashCommand;
import com.economy.commands.HudCommand;
import com.economy.commands.ItemInfoCommand;
import com.economy.commands.MoneyCommand;
import com.economy.commands.MyShopCommand;
import com.economy.commands.PlayerShopCommand;
import com.economy.commands.ShopCommand;
import com.economy.commands.ShopsCommand;
...
import com.economy.systems.BlockBreakRewardSystem;
import com.economy.systems.BlockInteractLogSystem;
import com.economy.systems.BlockPlaceTrackerSystem;
import com.economy.systems.EconomyHudSystem;
import com.economy.systems.MonsterKillRewardSystem;
...
public class Main extends JavaPlugin {
    ...
    protected void setup() {
        ...
        try {
            ...
            try {
                Class<?> rootInteractionClass = Class.forName("com.hypixel.hytale.server.core.modules.interaction.interaction.config.RootInteraction");
                ...
                java.lang.reflect.Method loadAssetsMethod = assetStore.getClass().getMethod("loadAssets", String.class, java.util.List.class);
        ...
        if (cassaforteAvailable) {
            if (com.economy.util.CassaforteEconomy.isAvailable()) {
                ...
                this.getLogger().at(Level.INFO).log("EconomySystem loaded successfully with Cassaforte (no economy provider registered - using own economy system)");
            }
        } else {
    ...
    private void checkVaultUnlocked() {
        try {
            // Tenta usar reflexão para verificar se VaultUnlocked está disponível
            // Isso permite que o código compile mesmo sem o JAR do VaultUnlocked
            ...
            Object pluginIdentifier = pluginIdentifierClass.getMethod("fromString", String.class).invoke(null, "TheNewEconomy:VaultUnlocked");
            Object wildcardRange = semverRangeClass.getField("WILDCARD").get(null);
            
            boolean hasPlugin = (Boolean) pluginManager.getClass().getMethod("hasPlugin", pluginIdentifierClass, semverRangeClass)
                    .invoke(pluginManager, pluginIdentifier, wildcardRange);
            
            if (hasPlugin) {
                // VaultUnlocked está instalado - registra o provedor de economia
                try {
                    // Obtém o VaultUnlockedServicesManager usando reflexão
                    Class<?> servicesManagerClass = Class.forName("net.milkbowl.vault2.VaultUnlockedServicesManager");
                    Object servicesManager = servicesManagerClass.getMethod("get").invoke(null);
                    
                    // Cria a instância do provedor de economia
                    com.economy.integration.VaultUnlockedEconomy economyProvider = 
                        new com.economy.integration.VaultUnlockedEconomy(this);
                    
                    // Registra o provedor usando o método economy()
                    // Seguindo: https://tne.gitbook.io/vaultunlocked/hytale/economy-provider
                    Class<?> economyInterfaceClass = Class.forName("net.milkbowl.vault2.economy.Economy");
                    servicesManagerClass.getMethod("economy", economyInterfaceClass)
                        .invoke(servicesManager, economyProvider);
                    
                    this.getLogger().at(Level.INFO).log("VaultUnlocked is installed, enabling VaultUnlocked support.");
                    this.getLogger().at(Level.INFO).log("TheEconomy registered as economy provider for VaultUnlocked.");
                } catch (Exception e) {
                    this.getLogger().at(Level.WARNING).log("Failed to register VaultUnlocked economy provider: " + e.getMessage());
                    e.printStackTrace();
                }
            } else {
                this.getLogger().at(Level.FINE).log("VaultUnlocked is not installed, disabling VaultUnlocked support.");
            }
        } catch (ClassNotFoundException e) {
            // VaultUnlocked classes não estão disponíveis - isso é normal se o JAR não estiver presente
            // Não loga nada, pois é esperado quando o JAR não está presente
        } catch (Exception e) {
            // Outros erros são silenciosamente ignorados
            // VaultUnlocked é opcional, então não queremos causar erros se não estiver disponível
        }
    }
    ...
    protected void start() {
        ...
        try {
            ...
            this.getLogger().at(Level.WARNING).log("EntityModule not available - entity systems will not be registered: %s", e.getMessage());
        }
